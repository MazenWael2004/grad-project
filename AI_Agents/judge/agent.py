...
# Wrap the distance calculator as an ADK FunctionTool so the judge LLM can call it
...
- "failed_constraints": list of strings (constraint violations, e.g. ["Budget", "Duration"])
- "spatial_issues": list of strings (spatial problems found, e.g. ["Day 1: Giza Pyramids to Luxor Temple is 650km - unreachable in same day"])
- "spatial_score": float from 0.0 to 1.0 (1.0 = perfect spatial coherence, 0.0 = completely incoherent)

Do not talk about the plan or the query beyond the evaluation.
