...
from .models import TourStateCheckpoint, TourSession
...
class DjangoCheckpointStore:
    def append(self, checkpoint: Checkpoint) -> bool:
        # Check if event_id already exists to prevent duplicate event processing
        if TourStateCheckpoint.objects.filter(session_id=checkpoint.session_id, event_id=checkpoint.event_id).exists():
            return False
            
        # Get the latest checkpoint for the session
        latest = TourStateCheckpoint.objects.filter(session_id=checkpoint.session_id).order_by("state_version").last()
        if latest is not None and checkpoint.state_version <= latest.state_version:
            raise StaleStateVersion("checkpoint version must be greater than the latest version")
            
        session = TourSession.objects.get(id=checkpoint.session_id)
        # Create a new checkpoint in DB
        TourStateCheckpoint.objects.create(
            id=checkpoint.state_payload.get("id", f"chk_{uuid4().hex}"),
            session=session,
